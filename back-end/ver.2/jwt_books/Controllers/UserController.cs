    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> UserCheckAuthentication([FromBody] LoginModel loginModel)
        {
            try
            {
                ...
                var jwt = new JwtToken();
                var tokenString = jwt.GenerateJwtToken(username: username, userId: userId, role: userRole);
        ...
        public async Task<IActionResult> GetUserById(int id)
        {
            try
            {
                ...
                using var command = new SqlCommand("SELECT UserID, Username, FullName, Email, Phone, Address FROM Users WHERE UserID = @UserId", connection);
            ...
            {
                // Ghi lại ngoại lệ (cân nhắc sử dụng một framework ghi log)
                return StatusCode(500, new { message = "An error occurred while processing your request.", details = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> GetUserByUsername(string username)
        {
            try
            {
                ...
                using var command = new SqlCommand("SELECT UserID, Username, FullName, Email, Address FROM Users WHERE Username = @Username", connection);
                command.Parameters.AddWithValue("@Username", username);
